...
# 出力動画のファイル名
CLIP_FPS = 40.0
output_video_path = "output_late6.mp4"
codec = cv2.VideoWriter_fourcc(*'mp4v')
video = cv2.VideoWriter(output_video_path, codec, CLIP_FPS, (width_original, height_original))

# VideoWriterオブジェクトの作成 (MP4形式、H.264コーデック、30fps)
...
while True:
    ...
    if tiestep==0:
        ...
        if lpoint[0]+50<rpoint[0] and cross == 0:
            ...
            cv2.putText(frame_original,"pass it behind",lpoint_out,0,0.8,(0,0,255),2,cv2.FILLED) 
        else:
            cross = 1
        image_path = f"img/img{tiestep+1}.jpg"  # 挿入したい画像のパス
        frame_original = embed_image(
        ...
        assistimg1 = cv2.imread('tieimg4.png')
        height_img1 ,width_img1 = assistimg1.shape[:2]
        y_img = 450
        x_img = 900
        frame_original[y_img:y_img+height_img1, x_img:x_img+width_img1] = assistimg1
        ...
        assistimg1 = cv2.imread('tieimg6.png')
        height_img1 ,width_img1 = assistimg1.shape[:2]
        y_img = 450
        x_img = 900
        frame_original[y_img:y_img+height_img1, x_img:x_img+width_img1] = assistimg1
        ...
        image_path = f"img/img{tiestep}.jpg"  # 挿入したい画像のパス
        frame_original = embed_image(
            frame_original, image_path, position=(frame_original.shape[1] - 250, 50), overlay_size=(180, 230),
            text="PERFECT!!", text_position=(frame_original.shape[1] - 280, 40), text_color=(0, 0, 0), text_size=1.3, text_thickness=3
        )
    else:
        cv2.putText(frame_original,"unknown",(40,80),0,0.8,(255,0,0),2,cv2.FILLED)  
    ...
    #out = cv2.VideoWriter(output_video_path, fourcc, 30.0, (frame_original.shape[1], frame_original.shape[0]))
    # 'q' キーを押すと終了
